...
def get_js_parsed_ast(file_content):
  resp = requests.post("http://127.0.0.1:3000/parse", data=file_content.encode("utf-8"), headers={"content-type":"text/plain"})
    ...
    with open(fullpath, 'r') as f:
      ...
      for identifier in identifiers:
        if identifier["type"] == "ExportNamedDeclaration":
          ...
          if main_dec["type"] == "FunctionDeclaration":
            tokens.append(main_dec["id"]["name"])
            tokens_dict[main_dec["id"]["name"]] = fullpath + ":" + str(main_dec["loc"]["start"]["line"])
          if main_dec["type"] == "VariableDeclaration":
            for declaration in main_dec["declarations"]:
              tokens.append(declaration["id"]["name"])
              tokens_dict[declaration["id"]["name"]] = fullpath + ":" + str(main_dec["loc"]["start"]["line"])
